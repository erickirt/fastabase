...
export class SupabaseStack extends FargateStack {
  ...
  readonly dashboardUserSecret: Secret;
  ...
  constructor(scope: Construct, id: string, props: cdk.StackProps & {
    ...
    const dbSecret = Secret.fromSecretCompleteArn(this, 'SupabaseAdminSecret', props.dbSecretArn);
    ...
    const postgresMetaImageUri = new cdk.CfnParameter(this, 'PostgresMetaImageUri', {
      type: 'String',
      default: 'public.ecr.aws/supabase/postgres-meta:v0.89.3',
    ...
    // const supabaseAdminSecret = db.genUserPassword(this, 'supabase_admin');
    const supabaseAdminSecret = dbSecret;
    /** Secret of supabase_auth_admin user */
    const supabaseAuthAdminSecret = db.genUserPassword(this, 'supabase_auth_admin');
    /** Secret of supabase_storage_admin user */
    const supabaseStorageAdminSecret = db.genUserPassword(this, 'supabase_storage_admin');
    /** Secret of authenticator user */
    const authenticatorSecret = db.genUserPassword(this, 'authenticator');
    /** Secret of dashboard user  */
    this.dashboardUserSecret = db.genUserPassword(this, 'dashboard_user');

    new cdk.CfnOutput(this, 'DashboardUserSecretArn', {
      value: this.dashboardUserSecret.secretArn,
    });
    ...
    new cdk.CfnOutput(this, 'AnonKeyParameterName', {
      value: anonKey.ssmParameter.parameterName,
    });
    ...
    new cdk.CfnOutput(this, 'ServiceRoleKeyParameterName', {
      value: serviceRoleKey.ssmParameter.parameterName,
    });
    ...
    loadBalancer.connections.allowFrom(ec2.Peer.prefixList(cfPrefixList.prefixListId), ec2.Port.tcp(80), 'CloudFront');
     ...
     */
    this.apiExternalUrl = `https://${cdn.distribution.domainName}`;

    new cdk.CfnOutput(this, 'ApiExternalUrl', {
      value: this.apiExternalUrl,
    });
    ...
    kong.connections.allowFrom(loadBalancer, ec2.Port.tcp(8100), 'ALB healthcheck');
    ...
    const authProviders = auth.addExternalAuthProviders(`${this.apiExternalUrl}/auth/v1/callback`, 3);
    ...
    realtime.connections.allowInternally(ec2.Port.allTraffic());
